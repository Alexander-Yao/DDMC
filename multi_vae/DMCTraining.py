...
class Trainer():
    def __init__(self, model, optimizer, optimizer2,cont_capacity=None,
                 disc_capacity=None, print_loss_every=50, record_loss_every=100,
                 use_cuda=False, view_num=2, DATA='DATA', num_clusters=10):
        """
        **Acknowledgments**
    ...
    def _train_iteration(self, input_data):
        """
        ...
        """
        self.num_steps += 1
        # print('input data shape', len(input_data), type(input_data))
        data = torch.split(input_data[0], 1, dim=1)
        data = [torch.squeeze(_, dim=1) for _ in data]
        # print('test', len(data), data[0].shape)
        for i in range(self.view_num):
            if self.use_cuda:
                # print(i, '======')
        ...
        Loss = []
        # print(recon_batchs[0].shape[0], '\ndafa',data[0].shape, self.model.num_pixels[0])
        # print(recon_batchs[0].view(recon_batchs[0].shape[0], -1).shape)
        # print(data[0].view(data[0].shape[0], -1).shape)

        # max_recon_loss = F.binary_cross_entropy(recon_batchs[0].view(-1, self.model.num_pixels[0]), data[0].view(-1, self.model.num_pixels[0]))
        max_recon_loss = F.binary_cross_entropy(recon_batchs[0].view(recon_batchs[0].shape[0], -1), data[0].view(data[0].shape[0], -1))
        ...
        for i in range(self.view_num):
            countinue_name = 'cont' + str(i+1)
            cluster_loss.append(self.cluster_loss(latent_dist[countinue_name]))
        for i in range(self.view_num - 1):
            cluster_loss[i].backward(retain_graph=True)
        cluster_loss[-1].backward()
    ...
    def _loss_function(self, data, recon_data, latent_dist, view=0, max_recon_loss=1000, beta=1):
        ...
        if self.model.Net == 'C':
            recon_loss = F.binary_cross_entropy(recon_data.view(recon_data.shape[0], -1),
    ...
    def cluster_loss(self, x):
        ...
        :return: loss: the average distance between samples and their responding centers
        """
        kmeans = KMeans(n_clusters=self.num_clusters, n_init=20)
        y_pred = kmeans.fit_predict(X=x[0].cpu().detach().numpy())
        centers = torch.tensor(kmeans.cluster_centers_.astype(np.float32))
        x_norm = torch.reshape(torch.sum(torch.square(x), 1), [-1, 1])  # column vector
        center_norm = torch.reshape(torch.sum(torch.square(centers), 1), [1, -1])  # row vector
